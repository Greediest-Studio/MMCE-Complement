...
/** Eight-tier fluid-to-energy machine input hatch. */
public class BlockLiquidEnergizerHatch extends BlockMachineComponent
    ...
    public BlockLiquidEnergizerHatch() {
        super(Material.IRON);
        Block block = this;
        ...
        block.setTranslationKey("mmce_complement.liquid_energizer_hatch");
        block.setCreativeTab(CommonProxy.creativeTabModularMachinery);
        setDefaultState(getDefaultState().withProperty(SIZE,
            FluidHatchSize.TINY));
    }
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state,
        ...
        if (!world.isRemote) {
            ...
            if (tile instanceof TileLiquidEnergizerHatch) {
                TileLiquidEnergizerHatch hatch =
                ...
                if (!held.isEmpty() && FluidUtil.getFluidHandler(held) != null) {
                    ...
                    player.openGui(MMCEComplement.instance,
                        MMCEComplement.GUI_LIQUID_ENERGIZER_HATCH, world,
    ...
    public void addInformation(@Nonnull ItemStack stack, @Nullable World world,
        ...
        FluidHatchSize tier = tierFromMeta(stack.getMetadata());
        tooltip.add(I18n.format(
            "tile.mmce_complement.liquid_energizer_hatch.tip.fluid_capacity",
            ModConfig.getLiquidEnergizerFluidCapacity(tier)));
        tooltip.add(I18n.format(
            "tile.mmce_complement.liquid_energizer_hatch.tip.energy_capacity",
            ModConfig.getLiquidEnergizerEnergyCapacity(tier)));
        tooltip.add(I18n.format(
            "tile.mmce_complement.liquid_energizer_hatch.tip.conversion"));
    }

    @Override
    public void getSubBlocks(CreativeTabs tab, NonNullList<ItemStack> items) {
        for (FluidHatchSize tier : FluidHatchSize.values()) {
    ...
    public IBlockState getStateFromMeta(int meta) {
        return getDefaultState().withProperty(SIZE, tierFromMeta(meta));
    }
    ...
    public String getIdentifierForMeta(int meta) {
        return tierFromMeta(meta).getName();
    }
    ...
    public Iterable<IBlockState> getValidStates() {
        ...
        for (FluidHatchSize tier : FluidHatchSize.values()) {
            states.add(getDefaultState().withProperty(SIZE, tier));
        }
        return states;
    }
    ...
    public TileEntity createNewTileEntity(@Nonnull World world, int meta) {
        return new TileLiquidEnergizerHatch(tierFromMeta(meta));
    }

    @Override @Nullable
    public TileEntity createTileEntity(@Nonnull World world,
                                       @Nonnull IBlockState state) {
        return new TileLiquidEnergizerHatch(state.getValue(SIZE));
    }

    private static FluidHatchSize tierFromMeta(int meta) {
        FluidHatchSize[] tiers = FluidHatchSize.values();
        return tiers[MathHelper.clamp(meta, 0, tiers.length - 1)];
    }
}
