...
/** Nine-isolated-tank input hatch; normal is the lowest available size. */
public class BlockNineFluidInputHatch extends BlockMachineComponent
    ...
    public BlockNineFluidInputHatch() {
        super(Material.IRON);
        Block block = this;
        ...
        block.setTranslationKey("mmce_complement.nine_fluid_input_hatch");
            ...
            if (tile instanceof TileQuadFluidInputHatch) {
                ...
                if (!held.isEmpty() && FluidUtil.getFluidHandler(held) != null) {
                    ...
                    player.openGui(MMCEComplement.instance,
                        MMCEComplement.GUI_NINE_FLUID_INPUT_HATCH,
    ...
    public void addInformation(@Nonnull ItemStack stack, @Nullable World world,
        ...
        tooltip.add(TextFormatting.GRAY + I18n.format(
            "tile.mmce_complement.nine_fluid_input_hatch.tip.capacity",
            size.getSize(), TileNineFluidInputHatch.capacityForTotal(size.getSize())));
        if (Mods.MEKANISM.isPresent()) {
            tooltip.add(TextFormatting.GRAY + I18n.format("tooltip.fluidhatch.tank.mek"));
        }
        tooltip.add(TextFormatting.GRAY + I18n.format(
            "tile.mmce_complement.nine_fluid_input_hatch.tip.isolated"));
        tooltip.add(TextFormatting.GRAY + I18n.format(
            "tile.mmce_complement.nine_fluid_input_hatch.tip.mixed"));
    ...
    private static TileEntity createHatchTile(FluidHatchSize size) {
        return Mods.MEKANISM.isPresent()
            ? NineFluidHatchMekanismFactory.create(size)
            : new TileNineFluidInputHatch(size);
    }
}
