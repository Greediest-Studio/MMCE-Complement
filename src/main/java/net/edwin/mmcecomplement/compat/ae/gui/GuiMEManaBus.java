...
public class GuiMEManaBus extends GuiContainer {
    ...
    private final TileMEManaBusBase tile;
    private GuiTextField bufferCapacityField;
    private long lastSentCapacity;

    public GuiMEManaBus(EntityPlayer player, TileMEManaBusBase tile) {
        super(new ContainerMEManaBus(player, tile));
        this.tile = tile;
        this.xSize = 176;
        this.ySize = 166;
    }
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        String title = tile.provideComponent().getIOType().name().equals("INPUT")
                ? I18n.format("gui.mmce_complement.me_mana_input_bus.title")
                : I18n.format("gui.mmce_complement.me_mana_output_bus.title");
        fontRenderer.drawString(title, 8, 6, 0x404040);
        fontRenderer.drawString(I18n.format("gui.mmce_complement.me_mana_bus.buffer") + ": "
            + formatLong(tile.getCurrentMana()) + " Mana", 8, 22, 0x404040);
