...
/** Mekanism gas-capable form of the combined data/item/fluid input hatch. */
public class TileDataItemInputHatchMekanism extends TileDataItemInputHatch
    implements IExtendedGasHandler, ITubeConnection {

    public TileDataItemInputHatchMekanism() {
        super();
    }

    public TileDataItemInputHatchMekanism(DataInputAssemblyTier tier) {
        super(tier);
    }
    ...
    protected HybridTank createTank(int capacity) {
        return new HybridGasTank(capacity) {
            ...
            protected void onContentsChanged() {
                TileDataItemInputHatchMekanism.this.onTankContentsChanged();
            }
        };
    }
    ...
    @Override
    public synchronized int receiveGas(EnumFacing side, GasStack stack,
                                       boolean doTransfer) {
        if (stack == null || stack.amount <= 0
            || conflictsWithFluid(stack.getGas())) {
            return 0;
        }
        for (int i = 0; i < tanks.length; i++) {
    ...
    @Nullable
    @Override
    public synchronized GasStack drawGas(EnumFacing side, int amount,
                                         boolean doTransfer) {
        if (amount <= 0) {
            return null;
        }
    ...
    @Nullable
    @Override
    public synchronized GasStack drawGas(GasStack stack,
                                         boolean doTransfer) {
        if (stack == null || stack.amount <= 0) {
            return null;
        }
    ...
    public GasTankInfo[] getTankInfo() {
        ...
        for (int i = 0; i < tanks.length; i++) {
            ...
            result[i] = info.length == 0 ? null : info[0];
        }
        return result;
    }
    ...
    private boolean conflictsWithFluid(Gas gas) {
        if (gas == null || !gas.hasFluid()) {
            return false;
        }
        for (HybridTank tank : tanks) {
            FluidStack fluid = tank.getFluid();
            if (fluid != null && fluid.amount > 0
                && fluid.getFluid() == gas.getFluid()) {
                return true;
            }
        }
        return false;
    ...
    public String getStoredDisplayName(int index) {
        ...
        GasStack gas = gasTank(index).getGas();
        return gas == null || gas.amount <= 0
            ? null : gas.getGas().getLocalizedName();
    }
