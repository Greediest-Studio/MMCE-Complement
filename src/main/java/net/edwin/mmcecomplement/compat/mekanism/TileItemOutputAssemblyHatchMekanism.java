...
/** Mekanism gas-capable form of the item/fluid output assembly. */
public class TileItemOutputAssemblyHatchMekanism extends TileItemOutputAssemblyHatch
    implements IExtendedGasHandler, ITubeConnection {
    public TileItemOutputAssemblyHatchMekanism() { super(); }
    public TileItemOutputAssemblyHatchMekanism(DataInputAssemblyTier tier) { super(tier); }

    @Override protected HybridTank createTank(int capacity) {
        return new HybridGasTank(capacity) {
            @Override protected void onContentsChanged() {
                TileItemOutputAssemblyHatchMekanism.this.markForUpdateSync();
            }
        };
    }
    ...
    @Nullable @Override public synchronized GasStack drawGas(EnumFacing side, int amount, boolean doTransfer) {
        ...
        for (int i = 0; i < tanks.length; i++) {
            GasStack stored = gasTank(i).getGas();
            if (stored != null && stored.amount > 0) return gasTank(i).drawGas(side, amount, doTransfer);
        }
        return null;
    }
    @Nullable @Override public synchronized GasStack drawGas(GasStack stack, boolean doTransfer) {
        ...
        for (int i = 0; i < tanks.length; i++) {
            GasStack stored = gasTank(i).getGas();
            if (stored != null && stored.amount > 0 && stored.isGasEqual(stack)) return gasTank(i).drawGas(stack, doTransfer);
        }
        return null;
    }
    @Override public synchronized boolean canReceiveGas(EnumFacing side, Gas gas) {
        ...
        for (int i = 0; i < tanks.length; i++) {
            GasStack stored = gasTank(i).getGas();
            if (stored != null && stored.amount > 0 && stored.isGasEqual(gas)) return gasTank(i).receiveGas(side, probe, false) > 0;
        }
        for (int i = 0; i < tanks.length; i++) if (!isTankOccupied(i)) return gasTank(i).canReceiveGas(side, gas);
        return false;
    }
    @Override public synchronized boolean canDrawGas(EnumFacing side, Gas gas) {
        for (int i = 0; i < tanks.length; i++) {
            GasStack stored = gasTank(i).getGas();
            if (stored != null && stored.amount > 0 && (gas == null || stored.isGasEqual(gas))) return gasTank(i).canDrawGas(side, gas);
        }
        return false;
    }
    ...
    @Override public boolean hasCapability(Capability<?> c, @Nullable EnumFacing f) { return isGasCapability(c) || super.hasCapability(c, f); }
    @Nullable @Override @SuppressWarnings("unchecked") public <T> T getCapability(Capability<T> c, @Nullable EnumFacing f) { return isGasCapability(c) ? (T) this : super.getCapability(c, f); }
    ...
    @Override protected void writeExtraTankNBT(int i, NBTTagCompound tag) { gasTank(i).writeGasToNBT(tag); }
