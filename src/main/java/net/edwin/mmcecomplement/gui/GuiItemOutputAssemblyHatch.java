...
/** GUI for the item/fluid output assembly, using the input assembly artwork. */
public class GuiItemOutputAssemblyHatch extends GuiContainer {
    private final TileItemOutputAssemblyHatch tile;
    private final ResourceLocation background;

    public GuiItemOutputAssemblyHatch(EntityPlayer player, TileItemOutputAssemblyHatch tile) {
        super(new ContainerItemOutputAssemblyHatch(player, tile));
        this.tile = tile;
        this.background = new ResourceLocation("mmce_complement",
            "textures/gui/container/input_assembly_" + tile.getTier().getName() + ".png");
        this.xSize = 256; this.ySize = tile.getTier().getGuiHeight();
    }
    ...
    @Override protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        fontRenderer.drawString(I18n.format("gui.mmce_complement.output_assembly_hatch.title." + tile.getTier().getName()), 8, 6, 0x404040);
    ...
    @Override protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        mc.getTextureManager().bindTexture(background); drawTexturedModalRect(guiLeft, guiTop, 0, 0, xSize, ySize);
        for (int i = 0; i < tile.getTankCount(); i++) drawTankContents(i, guiLeft + tankX(i), guiTop + tankY(i));
        GlStateManager.color(1F, 1F, 1F, 1F);
    }
    private void drawCentered(String text, int x, int y) { fontRenderer.drawString(text, x - fontRenderer.getStringWidth(text) / 2, y, 0x404040); }
    private void drawTankContents(int index, int x, int y) {
        ...
        int tint = tile.getStoredTint(index); GlStateManager.color(((tint >> 16) & 0xFF) / 255F, ((tint >> 8) & 0xFF) / 255F, (tint & 0xFF) / 255F, 1F);
        ...
        mc.getTextureManager().bindTexture(TextureMap.LOCATION_BLOCKS_TEXTURE); drawTexturedModalRect(x, y, sprite, width, getTankHeight()); GlStateManager.color(1F, 1F, 1F, 1F);
    }
    private int tankAt(int mouseX, int mouseY) { int rx = mouseX - guiLeft, ry = mouseY - guiTop; for (int i = 0; i < tile.getTankCount(); i++) if (rx >= tankX(i) && rx < tankX(i) + getTankWidth() && ry >= tankY(i) && ry < tankY(i) + getTankHeight()) return i; return -1; }
