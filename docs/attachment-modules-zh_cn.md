...
附属模块检查现在采用“变化触发 + 定期兜底”策略。一次成功检查后，只要附属区域没有方块或区块更新，就会复用上次的匹配结果，不再在每个 MMCE 结构检查周期重新执行昂贵的模块成型匹配。区域更新会立即使缓存失效；默认每 100 tick 进行一次兜底检查，用于捕获事件监听窗口之外的改动。这样可以降低待机机器的开销，同时不会延迟正常的搭建或拆除响应。若附属区域暂时未加载，控制器会保留上一次有效状态，待区域重新加载后再检查。
...
模块状态变化只在机器结构检查任务中解析；同一次结构检查还会预先生成该机器所有模块配方条件的只读结果。配方开始、重启以及运行中的每个 tick 只读取这个结果，不再复制活动模块集合或执行模块成型判断。运行途中失去必需模块或出现禁止模块时，结构检查刷新结果后会返回相应失败状态。
